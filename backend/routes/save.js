...
router.post("/save-report", authMiddleware, async (req, res) => {
  try {
    ...
    console.log("📥 Save report request from:", email);
    ...
    console.log("🔍 Extracted data - imagePath:", imagePath);
    console.log("🔍 Extracted data - predictedClass:", predicted_class);
    console.log("🔍 Extracted data - patientData keys:", patientData ? Object.keys(patientData) : "null");
    ...
    if (!imagePath) {
      console.warn("⚠️ imagePath is missing!");
      return res.status(400).json({
        status: "error",
    ...
    if (error.code === 11000) {
      return res.status(400).json({
        status: "error",
        message: "This patient already has a saved report. Please create a new patient record to save another report.",
        details: "Duplicate patient ID"
      });
    }
