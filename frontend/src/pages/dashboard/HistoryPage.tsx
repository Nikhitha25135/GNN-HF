...
export default function HistoryPage() {
  ...
  // ============================================================
  // ⭐ VIEW FULL REPORT (with imageBase64)
  // ============================================================
  const viewFullReport = async (id: string) => {
    try {
      const res = await fetch(`${API_ENDPOINTS.GET_REPORT(id)}`, {
        headers: getAuthHeaders(),
      });
              ...
              <div className="p-4 bg-muted/30 rounded-xl">
                ...
                <div className="rounded-xl overflow-hidden border border-border">
                  {selectedRecord.imageBase64 ? (
                    ...
                    <img
                      src={`${import.meta.env.VITE_API_URL}/uploads/${selectedRecord.imagePath.split('/').pop()}`}
                      className="w-full h-48 object-contain bg-muted/50"
                      alt="ECG Image"
                      onError={(e) => {
                    ...
                    <div className="w-full h-48 bg-muted/50 flex items-center justify-center text-muted-foreground">
                      No image available
                    </div>
                  )}
                </div>
